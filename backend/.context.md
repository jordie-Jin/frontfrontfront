...
* `test(company)` 기업 검색 서비스 테스트를 최신 시그니처/DTO에 맞게 수정
...
* `feat(web)` 프런트 기준 기업/대시보드/모델 API 엔드포인트 및 응답 DTO 추가
* `refactor(company)` 기업 검색 파라미터/응답 포맷을 프런트 스펙으로 맞춤
...
* **2026-02-02**
    ...
    * 전체 통합 테스트 정상화
    * 프런트 기준 기업/대시보드/모델 API 응답 스키마 정합화
